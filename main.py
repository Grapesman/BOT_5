...
async def on_startup(dp):
    ...
    await bot.send_message(chat_id=settings.TG_NOTIFICATION_ID, text = "Добро пожаловать, выберете действие ниже.", reply_markup=keyboard)

async def on_shutdown(dp):
    await bot.send_message(chat_id=settings.TG_NOTIFICATION_ID, text = "В настоящее время П5 БОТ не работает. Как только работа восстановится - мы вас оповестим.")
